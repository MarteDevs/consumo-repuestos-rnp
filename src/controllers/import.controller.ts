...
export const importExcelData = async (req: Request, res: Response) => {
  try {
    const filePath = path.join(__dirname, '../../temp/POOT-2025-ACTUAL.xlsx');
    ...
    await prisma.query1.deleteMany({});
    ...
    for (let i = 0; i < dataToInsert.length; i += batchSize) {
      ...
      await prisma.query1.createMany({
        data: batch,
        skipDuplicates: true
      });
...
export const searchByPoot = async (req: Request, res: Response) => {
  try {
    ...
    const results = await prisma.query1.findMany({
      where: {
        poot_b: {
...
export const searchByEquipo = async (req: Request, res: Response) => {
  try {
...
export const getImportStats = async (req: Request, res: Response) => {
  try {
    const total = await prisma.query1.count();
    const lastImport = await prisma.query1.findFirst({
